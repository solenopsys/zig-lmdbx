...
export fn lmdbx_open(path: [*:0]const u8, db_ptr: *?*anyopaque) c_int {
    const db = c_allocator.create(lmdbx.Database) catch return -1;
...
export fn lmdbx_close(db_ptr: ?*anyopaque) void {
    if (db_ptr) |ptr| {
        const db: *lmdbx.Database = @ptrCast(@alignCast(ptr));
        db.close();
        c_allocator.destroy(db);
    }
}
...
export fn lmdbx_put(db_ptr: ?*anyopaque, key: [*]const u8, key_len: usize, value: [*]const u8, value_len: usize) c_int {
    const db: *lmdbx.Database = @ptrCast(@alignCast(db_ptr orelse return -1));
...
export fn lmdbx_get(db_ptr: ?*anyopaque, key: [*]const u8, key_len: usize, value_ptr: *?[*]u8, value_len: *usize) c_int {
    const db: *lmdbx.Database = @ptrCast(@alignCast(db_ptr orelse return -1));
    const k = key[0..key_len];
    const result = db.get(c_allocator, k) catch return -1;
...
export fn lmdbx_txn_abort(db_ptr: ?*anyopaque) void {
    const db: *lmdbx.Database = @ptrCast(@alignCast(db_ptr orelse return));
    db.abortTransaction();
}

export fn lmdbx_cursor_open(db_ptr: ?*anyopaque, cursor_ptr: *?*anyopaque) c_int {
    const db: *lmdbx.Database = @ptrCast(@alignCast(db_ptr orelse return -1));
    const cursor_obj = c_allocator.create(lmdbx.Cursor) catch return -1;
    cursor_obj.* = db.openCursor() catch {
        c_allocator.destroy(cursor_obj);
        return -1;
    };
...
export fn lmdbx_cursor_close(cursor_ptr: ?*anyopaque) void {
    if (cursor_ptr) |ptr| {
        const cursor: *lmdbx.Cursor = @ptrCast(@alignCast(ptr));
        lmdbx.Database.closeCursor(cursor.*);
        c_allocator.destroy(cursor);
    }
}
...
export fn lmdbx_cursor_get(
    ...
    const cursor: *lmdbx.Cursor = @ptrCast(@alignCast(cursor_ptr orelse return -1));
